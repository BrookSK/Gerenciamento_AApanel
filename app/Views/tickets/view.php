...
<body>
    ...
    <div>
        <strong>Atribuído (user_id):</strong> <?php echo htmlspecialchars((string)($ticket['assigned_user_id'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>
    </div>

    <form method="post" action="/tickets/assign">
        <input type="hidden" name="ticket_id" value="<?php echo (int)$ticket['id']; ?>">
        <button type="submit">Pegar para mim</button>
    </form>
            ...
            <?php if (count($atts) > 0) : ?>
                <div style="margin-top: 8px;">
                    ...
                    <?php foreach ($atts as $a) : ?>
                        <div>
                            <a href="/tickets/attachment?id=<?php echo (int)$a['id']; ?>">
    ...
    <form method="post" action="/tickets/upload" enctype="multipart/form-data">
        <input type="hidden" name="ticket_id" value="<?php echo (int)$ticket['id']; ?>">
        <div>
            <textarea name="message" rows="5" cols="60" required></textarea>
        </div>
